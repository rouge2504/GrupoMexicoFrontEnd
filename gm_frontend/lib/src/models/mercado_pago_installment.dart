class MercadoPagoInstallment {
  ...
  static List<MercadoPagoInstallment> fromJsonList(List<dynamic> jsonList) {
    List<MercadoPagoInstallment> toList = [];

    jsonList.forEach((item) {
      MercadoPagoInstallment model = MercadoPagoInstallment.fromJson(item);
      toList.add(model);
    });
  ...
  MercadoPagoInstallment.fromJson(Map<String, dynamic> json) {
    installments = (json['installments'] != null)
        ? int.parse(json['installments'].toString())
        : -1;
    installmentRate = (json['installment_rate'] != null)
        ? double.parse(json['installment_rate'].toString())
        : -1;
